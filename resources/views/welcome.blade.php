    ...
    <div class="w-full max-w-6xl px-4">
        ...
        <div class="flex flex-wrap justify-around">
            ...
            <form action="{{ route('file.upload') }}" method="post" enctype="multipart/form-data" class="mb-4 w-full md:w-1/3 px-4 ">
                ...
                <input class="bg-green-600 mt-2 text-white rounded-md px-3 py-1 font-medium text-lg w-full"
                    type="submit" value="Upload">
                {{-- @dump($errors->first()) --}}
            </form>

            <form action="{{ route('file.ServiceSummaryUpload') }}" method="post" enctype="multipart/form-data" class="mb-4 w-full md:w-1/3 px-4">
                @csrf
                <input type="hidden" name="import_type" value="service_summary">
            ...
            <form action="{{ route('file.ChargesSummaryUpload') }}" method="post" enctype="multipart/form-data" class="mb-4 w-full md:w-1/3 px-4">
                @csrf
                <h3 class="text-lg font-bold mt-2">3. Charges Summary</h3>
                <input class="mt-1 text-lg w-full border rounded-md px-3 py-1" type="file" name="csv_file">
                <input class="bg-green-600 mt-1 text-white rounded-md px-3 py-1 font-medium text-lg w-full"
                    type="submit" value="Upload">
                {{-- @dump($errors->first()) --}}
            </form>

            <form action="{{ route('usagefile.upload') }}" method="post" enctype="multipart/form-data"
